...
class RomFolderScanner(RomScannerStrategy):
    ...
    def scan(self):
        ...
        num_removed_roms = self._removeDeadRoms(roms)        
        ...
        items = self._getNewItems(launcher_report)
        num_items = len(items)
        new_roms = self._processFoundItems(items, roms, launcher_report)
        ...
        log_info('Files checked     {0:6d}'.format(num_items))
    ...
    def _getNewItems(self, launcher_report):
...
class SteamScanner(RomFolderScanner):
    ...
    def _getNewItems(self, launcher_report):
    #def _scanForNewItems(self, roms, steamGames):
        
        if steamGames is None or len(steamGames) == 0:
        ...
        for steamGame in steamGames:
            ...
            self._updateProgress(i * 100 / num_games, steamGame['name'])
            i += 1
        ...
        return foundGames
        return super(SteamScanner, self)._getNewItems(launcher_report)

    def _getGamesFromLibrary(self):
    ...
    # --- Remove dead entries -----------------------------------------------------------------
    def _removeDeadRoms(self, roms, steamGames):
        ...
        steamGameIds = set(steamGame['appid'] for steamGame in steamGames)
    ...
    def scan(self):
        ...
        # --- Open ROM scanner report file ---
        launcher_report = self.reports_dir.pjoin(self.launcher['m_name'] + '_report.txt')
        log_info('Report file path "{0}"'.format(launcher_report.getOriginalPath()))
        launcher_report.open('w')
        ...
        launcher_report.write('Loading Steam titles ...\n')
        steamGames = self._getGamesFromLibrary()

        num_steamtitles = len(steamGames)
        launcher_report.write('{0} Steam titles currently in account\n'.format(num_steamtitles))
        log_info('Steam account contains {0} titles'.format(num_steamtitles))

        log_info('Removing dead ROMs ...')
        launcher_report.write('Removing dead ROMs ...\n')


        num_removed_roms = self._removeDeadRoms(roms, steamGames)        
        ...
        ########
        self._cacheCurrentAssets()
        
        log_info('Scanning new items from Steam')
        launcher_report.write('Scanning new items ...\n')

        newSteamGames = self._scanForNewItems(roms, steamGames)
        num_new_items = len(newSteamGames)

        log_info('New games scanner found {0} items'.format(num_new_items))
        launcher_report.write('  Games scanner found {0} items\n'.format(num_new_items))
                
        # ~~~ Now go processing new item by item ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        self._startProgressPhase('Advanced Emulator Launcher', 'Processing new items')
        log_debug('============================== Processing ROMs ==============================')
        launcher_report.write('Processing new items ...\n')
        num_items_added = 0

        for newSteamGame in newSteamGames:
            self._updateProgress(num_items_checked * 100 / num_new_items)

            newRom = self._processNewItem(newSteamGame, launcher_report)
            if not newRom:
                return None
        ...
        launcher_report.write('********** ROM scanner finished **********\n')
        launcher_report.write('Removed dead ROMs {0:6d}\n'.format(num_removed_roms))
        launcher_report.write('Games on steam    {0:6d}\n'.format(num_steamtitles))
        launcher_report.write('Games added       {0:6d}\n'.format(num_items_added))
    ...
    def _processNewItem(self, steamGame, launcher_report):
        ...
        for scraper in self.scrapers:
            ...
            #rom_name_scraping = text_format_ROM_name_for_scraping(romPath.getBase_noext())
            scraper.Scrape(steamGame['name'], None, romdata)
            
            if self._isProgressCanceled():
                self._endProgressPhase(True)
                return None

        #romdata     = self._roms_process_scanned_ROM(ROM, progress_number)
