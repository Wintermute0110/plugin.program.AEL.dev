 ...
 * All ROMs belonging to the same platform share the same artwork.
 ...
 * If option `ArtNextToROMs` then the RAD and the `<ROM_asset_path>` are ignored.
...
<launcher>
  ...
  <ROM_Options>ArtNextToROMs</ROM_Options>
...
<launcher>
  ...
  <!-- Use the ArtNextToROMs or <ROM_asset_path> 
       If custom subdirectory names then use <subpath_xxxxx>
  -->
  <ROM_Options>ArtNextToROMs</ROM_Options>
  <ROM_asset_path>/home/kodi/AEL-assets/sony-psx/</ROM_asset_path>
  <subpath_title />
...
#### Examples of Launcher XML saving/backup

 * When saving a Launcher in XML the full metadata tags `<year>`, etc. and the absolute artwork tags `<s_xxxxx>` are always used.

 * When saving an XML backup of all Categories and Launchers the full metadata and absolute artwork tags are used.

### Implementation algorithms

 * Keep in mind standalone launchers and special launchers in future AEL.
...
#### ROM scanner saving scrapped artwork

#### Importing a Category XML file

#### Saving a Category XML file
